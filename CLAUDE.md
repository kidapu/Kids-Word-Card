...
- 読み上げの文字列は `letters.js` の `sayOf` で作る。**表示と読み上げは別物**。
  - アルファベットは**大文字のまま渡すと iOS が「Capital A」と読む**ので、
    読み上げには小文字を渡す（表示は大文字のまま）。
  - 小文字にすると単語として読まれる `A`（冠詞）と `I`（代名詞）は、
    `SAY` で綴り（`ay` / `eye`）を指定している。
  - 「を」「ん」は1文字だと読み飛ばされることがあるので、同じく `SAY` で補う。
  - 他にもおかしく読まれる文字が見つかったら `SAY` に足す。
